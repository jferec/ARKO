...
main:
	...
	sw $s0, x1		#zapisz pod zmienną x1
...
as1:
	...
	sub $t0, $s0, $s2	# x1-x2
	sub $t1, $s3, $s1	# y2-y1
	sll $t0, $t0, 8		# przesunięcie licznika o scale w lewo przed operacją dzielenia
	div $s6, $t0, $t1	# a1 - wspolczynnik nachylenia pierwszej symetralnej (s6)

	add $t0, $s1, $s3	# y1+y2
	add $t1, $s0, $s2	# x2+x1
	# przesunięcie jednego czynnika o scale w prawo, tam gdzie są współrzędne, nie tracimy żadnej dokładności, zachowujemy dokładność na współczynniku a, więc nie przesuwamy $s6
	sra $t1, $t1, 8
	mul $t8, $t1, $s6
	sub $t8, $t0, $t8
...
as2:
	...
	add $t0, $s5, $s3	# y2+y3
	add $t1, $s4, $s2	# x2+x3
	# przesunięcie jednego czynnika o scale w prawo, tam gdzie są współrzędne, nie tracimy żadnej dokładności, zachowujemy dokładność na współczynniku a, więc nie przesuwamy $s7
	sra $t1, $t1, 8
	mul $t9, $t1, $s7
	sub $t9, $t0, $t9
...
circle:
	...
	mul $t0, $s6, $t4
	sra $t0, $t0, 8 	# przesunięcie wyniku mnożenia o scale w prawo
...
#LICZENIE PIERWIASTKA Z ALGORYTMU NEWTONA
	...
	beqz $t2, result 	#warunek początkowy pętli
pentla:
 #algorytm Newtona
    	sll $t1, $t0, 8
    	div $t1, $t1, $t6  
    	...
    	sra $t6, $t6, 1
   	addi $t3, $t3,  256
   	blt $t3, $t2, pentla	#warunek wyjścia z pętli

		
result:	#ZAMIANA Z FIXED POINT DO INT, ORAZ ZAOKRĄGLANIE, DRUKOWANIE OBLICZONYCH Xs, Ys, R
	
	andi $t0, $t4, 128 # jeśli >= .5 to zaokrąglamy
	add $t4, $t4, $t0 
	sra $t4, $t4, 8 #FIXED POINT -> INT Xs

	andi $t0, $t5, 128 # jeśli >= .5 to zaokrąglamy
	add $t5, $t5, $t0 
	sra $t5, $t5, 8	# Ys

	andi $t0, $t6, 128 # jeśli >= .5 to zaokrąglamy
	add $t6, $t6, $t0 
	sra $t6, $t6, 8 # R
...
starting_point:
	...
	#nie ma warunku wejscia do pętli ponieważ zawsze narysujemy jakieś punkty, jeśli R>0
color_loop:
	...
	# OBLICZANIE RÓWNANIA X^2+Y^2-R^2 DLA PIKSELA NA SKOS PRAWO-DÓŁ
	subi $t3, $t8, 1 # (y-1)
	mul $t3, $t3, $t3 # (y-1)^2
	add $s1, $t2, $t3 # (x+1)^2 + (y-1)^2
	sub $s1, $s1, $t1 # (x+1)^2 + (y-1)^2 - R^2
	abs $s1, $s1 	#wartość bezwzględna (ponieważ to równanie może dać wynik <0)
...
go_right_down:
	...
	blt $t8, $t7, triangle 	#WARUNEK KOŃCZĄCY PĘTLE
	b color_loop
...
loop_begin:
	ble $t1, $t2, step1     # jeśli x1 <= x2 kx = 3 
	li $s6, -3		# jeśli x1 > x2	kx = -3
	b step2			
step1:
	li $s6, 3		# jeśli x1 <= x2 kx = 3 
	
step2:
	ble $s1, $s2, step3     #jeśli y1 <= y2 ky = + linijka bajtów
	sub $s7, $zero, $t6	#jeśli y1 > y2 ky - linijka bajtów
	b step4
step3:
	add $s7, $zero, $t6	#jeśli y1 <= y2 ky = + linijka bajtów

step4:

	sub $t5, $t2, $t1	#dx = x2 - x1
	abs $t5, $t5		#wartość bezwzględna z dx


	sub $s0, $s2, $s1	#dy = y2 - y1
	abs $s0, $s0		#wartość bezwzględna z dy
...
step5:		
	...
	blt $t5, $s0, step6  	#jeśli dx<dy
	
	srl $s4, $t5, 1		#jeśli dx>=dy	e = dx/2 
	
	bge $t7, $t5, end_loops	# jeśli licznik >= dx kończ pętle

loop1:	
	add $t9, $t9, $s6        # p = p + kx (przesuwamy się w prawo o kx - 1 piksel), p to adres piksela na ktory wskazujemy
	sub $s4, $s4, $s0        # e = e - dy (dekrementacja e o dy)
	
	
	bgez $s4, step7	 	#jeśli e>=0
				#jeśli e<0
	add $t9, $t9, $s7        # p = p + ky (przesuwamy się o piksel do góry)
	add $s4, $s4, $t5        # e = e + dx
...
step7:	
	...
	subi $t9, $t9, 2	
        ...
        #while
        blt $t7, $t5, loop1	#jeśli licznik < dx skacz do loop1
        b end_loops		#jeśli licznik >= dx skacz na koniec
        
step6:	# jeśli dx<dy

	srl $s4, $s0, 1		#e = dy/2 
	
	bge $t7, $s0, end_loops	#jeśli licznik>= dy skacz na koniec
loop2:
	add $t9, $t9, $s7       # p = p + ky (piksel do góry)
	sub $s4, $s4, $t5       # e = e - dx (dekrementacja o dx)
	
	bgez $s4, step8		#jeśli e>=0
				#jeśli e<0
	add $t9, $t9, $s6 	# p = p + kx 
	add $s4, $s4, $s0       # e = e + dy
...
step8:	# jeśli e>0
	...
	subi $t9, $t9, 2	
        ...
        blt $t7, $s0, loop2	#jeśli licznik < dy skacz do loop2
       				#jeśli licznik >= dy idziesz na koniec
...
end_loops: 
	...
	bne $t3, 1, step9	#jeśli licznik jest różny od 1 sprawdź dalej
				#jeśli licznik jest równy 1 ładuj DRUGI BOK
	lw $t1, x2
	lw $s1, y2
	lw $t2, x3
	lw $s2, y3
...
step9:				#jeśli licznik jest różny od 1 sprawdź dalej
 	
 	bne $t3, 2, zapisz_plik	#jeśli licznik różny od 2 KOŃCZ
				#jeśli licznik jest równy 2 ładuj OSTATNI BOK
	lw $t1, x3
	lw $s1, y3
	lw $t2, x1
	lw $s2, y1
